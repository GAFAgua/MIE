...
function tintModel(root) {
  const grayClay = new THREE.MeshStandardMaterial({
    color: 0x808584,
    metalness: 0.02,
    roughness: 0.68,
...
function setAnnotation(name) {
  ...
  if (flowRoot) {
    flowRoot.children.forEach((child) => {
      child.visible = name === "none" || child.name === name || child.name === "global";
    });
  }
}
...
function createFeatureAnnotations() {
  ...
  addTube(outline, [
    [0, 1.72], [0.42, 1.63], [0.84, 1.39], [1.2, 0.94], [1.5, 0.24], [1.52, -0.34],
    [1.28, -0.98], [0.78, -1.48], [0.25, -1.66], [-0.25, -1.66], [-0.78, -1.48],
    [-1.28, -0.98], [-1.52, -0.34], [-1.5, 0.24], [-1.2, 0.94], [-0.84, 1.39],
    [-0.42, 1.63], [0, 1.72],
  ], 0.0038);

  const foil = makeLineGroup("foil");
  addTube(foil, [[-1.42, 1.08], [-1.54, 0.42], [-1.49, -0.34], [-1.22, -1.04]], 0.004);
  addTube(foil, [[1.42, 1.08], [1.54, 0.42], [1.49, -0.34], [1.22, -1.04]], 0.004);
  addBox(foil, 1.46, 0.05, 0.16, 1.34);

  const relief = makeLineGroup("relief");
  [
    [[-1.12, 0.82], [-0.82, 0.7], [-0.56, 0.52], [-0.38, 0.32]],
    [[-1.24, 0.56], [-0.92, 0.43], [-0.66, 0.22], [-0.48, -0.04]],
    [[-1.25, 0.26], [-0.9, 0.1], [-0.68, -0.18], [-0.56, -0.5]],
    [[1.12, 0.82], [0.82, 0.7], [0.56, 0.52], [0.38, 0.32]],
    [[1.24, 0.56], [0.92, 0.43], [0.66, 0.22], [0.48, -0.04]],
    [[1.25, 0.26], [0.9, 0.1], [0.68, -0.18], [0.56, -0.5]],
    [[-0.42, 0.78], [-0.16, 0.65], [0, 0.58], [0.16, 0.65], [0.42, 0.78]],
    [[-0.34, 0.52], [-0.12, 0.38], [0, 0.31], [0.12, 0.38], [0.34, 0.52]],
    [[-0.26, 0.24], [-0.08, 0.1], [0, 0.04], [0.08, 0.1], [0.26, 0.24]],
  ].forEach((line) => addTube(relief, line, 0.003));
  addBox(relief, -1.05, 1.02, 0.52, 0.22);
  addBox(relief, 1.05, 1.02, 0.52, 0.22);

  const ram = makeLineGroup("ram");
  addTube(ram, [[-0.46, 1.43], [-0.72, 1.33], [-0.78, 1.08], [-0.52, 0.98], [-0.3, 1.1]], 0.0038);
  addTube(ram, [[0.46, 1.43], [0.72, 1.33], [0.78, 1.08], [0.52, 0.98], [0.3, 1.1]], 0.0038);
  addBox(ram, -0.32, 1.28, 0.24, 0.18);
  addBox(ram, 0.32, 1.28, 0.24, 0.18);
  addTube(ram, [[-0.2, 1.16], [0, 0.86], [0.2, 1.16]], 0.0038);
  addBox(ram, 0, 1.28, 0.74, 0.6);

  const symmetry = makeLineGroup("symmetry");
  addTube(symmetry, [[0, 1.68], [0, 0.92], [0, 0.2], [0, -0.62], [0, -1.62]], 0.0032);
  addTube(symmetry, [[-0.18, 1.14], [-0.68, 0.9], [-0.95, 0.38], [-0.78, -0.36], [-0.44, -1.22]], 0.0032);
  addTube(symmetry, [[0.18, 1.14], [0.68, 0.9], [0.95, 0.38], [0.78, -0.36], [0.44, -1.22]], 0.0032);
  addBox(symmetry, -0.72, 0.18, 0.7, 1.5);
  addBox(symmetry, 0.72, 0.18, 0.7, 1.5);
